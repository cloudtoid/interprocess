...
Ready messages use the nonblocking path immediately. Empty queues retry on a one-millisecond timer without occupying libuv workers. Idle-to-active delivery can incur that polling interval, and a busy event loop can delay timers and cancellation. Pending timers add periodic CPU work while idle; use `AbortSignal` to stop unused receives.
