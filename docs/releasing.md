...
NuGet uses the explicit `VersionPrefix` in `src/dotnet/Interprocess/Interprocess.csproj`, starting at `3.1.0`. Bump it in the PR that releases .NET changes: `3.1.1` for the next patch, or `3.2.0` for the next minor release. Leave it unchanged for releases that only affect other languages. NuGet's `--skip-duplicate` prevents those releases and retries from publishing another .NET package. Version bumps are explicit; there is no automatic change detector or counter. The workflow remains `publish.yml` to preserve its NuGet trusted-publisher identity.
