...
NuGet publishes only when the library, its packaged README, or shared .NET build/dependency settings differ from the source commit recorded in the latest stable NuGet package. Test-only, benchmark, native-language, and workflow changes do not trigger a NuGet release. Set `VersionPrefix` in `src/dotnet/Interprocess/Interprocess.csproj` to `major.minor.0` (currently `3.1.0`). The release workflow selects the next published patch in that line: `3.1.0`, `3.1.1`, and so on; changing the prefix to `3.2.0` starts again at zero. Retries skip already published source changes. Registry or source-history lookup failures stop the release instead of guessing a version. The workflow remains `publish.yml` to preserve its NuGet trusted-publisher identity.
